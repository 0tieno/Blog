...
Authentication sessions are often stored in Redis so that repeated API calls can validate users quickly without hitting the main database each time.
...
If you want the protocol-level side of this topic, read the HTTP caching post in the series as well.
